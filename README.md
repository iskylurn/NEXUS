...
```
-  ## 4. The "Magic" Tweak (Custom RAM) :
By default, Nexus asks for ~4GB RAM per thread. If you want to run 8 threads, your PC or VPS might crash. Let's fix that.
...
If you don't change this number, your node will likely give an "Out of Memory" error or simply fail to start when you use the --max-threads command. By lowering this value, we "trick" the node into starting with less RAM, allowing our 8 threads to run smoothly on standard VPS setups.
